...
from models import (
    ...
    APIKeyRefresh, AnthropicChatRequest, EmbeddingRequest, RerankRequest,
    ImageGenerationRequest, TTSRequest, STTRequest, AudioUploadRequest,
    VideoGenerationRequest, VideoStatusRequest, BatchRequest, FileUploadResponse
)
from utils import (
...
async def anthropic_chat_completions(request: Request, background_tasks: BackgroundTasks):
    """Anthropic Claude chat completions endpoint"""
    # Check API_KEY if configured
    if API_KEY is not None:
        auth_header = request.headers.get("Authorization")
        if not auth_header or not auth_header.startswith("Bearer "):
...
async def image_generation(request: Request, background_tasks: BackgroundTasks):
...
async def speech_to_text(request: Request, background_tasks: BackgroundTasks):
...
async def upload_reference_audio(request: Request, background_tasks: BackgroundTasks):
    ...
    try:
        ...
        for key, value in form_data.items():
            if hasattr(value, 'read'):  # File upload
                data.add_field(key, value, filename=getattr(value, 'filename', 'audio'))
            else:
...
@app.get("/v1/audio/reference")
async def list_reference_audios(request: Request):
    """List reference audios endpoint"""
    # Get a key from the pool
    db = await AsyncDBPool.get_instance()
    keys = await db.get_key_list()
...
@app.post("/v1/audio/reference/delete")
async def delete_reference_audio(request: Request, background_tasks: BackgroundTasks):
    """Delete reference audio endpoint"""
    # Check API_KEY if configured
    if API_KEY is not None:
        request_api_key = request.headers.get("Authorization")
        if request_api_key != f"Bearer {API_KEY}":
...
async def video_generation(request: Request, background_tasks: BackgroundTasks):
...
async def video_status(request: Request, background_tasks: BackgroundTasks):
    """Get video generation status endpoint"""
    # Check API_KEY if configured
    if API_KEY is not None:
        request_api_key = request.headers.get("Authorization")
        if request_api_key != f"Bearer {API_KEY}":
...
async def upload_file(request: Request, background_tasks: BackgroundTasks):
    ...
    try:
        ...
        for key, value in form_data.items():
            if hasattr(value, 'read'):  # File upload
                data.add_field(key, value, filename=getattr(value, 'filename', 'file'))
            else:
                data.add_field(key, value)
...
async def get_batch(batch_id: str, request: Request):
    """Get batch processing job details"""
    # Get a key from the pool
    db = await AsyncDBPool.get_instance()
    keys = await db.get_key_list()
...
async def cancel_batch(batch_id: str, request: Request, background_tasks: BackgroundTasks):
